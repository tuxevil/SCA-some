...
Public Class SuperClase
    ...
    Dim numeroCot As Integer = 1


    ' Carga las configuraciones del archivo TecDev.ini
    Public Sub CargaOp()

        Try
            Dim Fnum As Integer = FreeFile() 'retorna el proximo numero de archivo a ser usado, es como fflush
            Dim T As String
            ...
            While Not EOF(Fnum)
                ...
                T = LineInput(Fnum)
                If T = "--sql_autos--" Then
                    '                    provider_sql = "Provider=SQLOLEDB;Data Source=" & LineInput(Fnum) & ";Initial Catalog=conAutomotriz;Persist Security Info=True;User ID=sa;Password=gotechdev"
                    '      
                    gVar.Nombre_Server = LineInput(Fnum)
                    provider_sql = "Provider=SQLOLEDB;Data Source='" & gVar.Nombre_Server & "';Initial Catalog=conAutomotriz;Persist Security Info=True;User ID=sa;Password=gotechdev"
                ...
                End If
                '--Reporte_Ventas--
                If T = "--Reporte_Ventas--" Then
                    gVar.reporte_ventas = LineInput(Fnum)
                End If

                If T = "--Reporte_Cotizaciones--" Then
                    gVar.reporte_cotizaciones = LineInput(Fnum)
                End If

                If T = "--Reporte_Solicitudes--" Then
                    gVar.reporte_solicitudes = LineInput(Fnum)
                End If

                If T = "--fechas--" Then
                    gVar.fecha_orden = LineInput(Fnum)
    ...
    'hace un algoritmo matematico
    Public Function CualCod(ByVal X As String) As Double
        Dim Z As Integer
        Z = 1
        Do While Z < Len(X)
            If Mid(X, Z, 1) = "-" Then
                Return CDbl(Mid(X, 1, Z - 1))
                Exit Function
            End If
            Z = Z + 1
        Loop
    ...
    Public Sub llenaComboVehiculosModelos(ByVal modelos As ComboBox)
        ...
        Db.Open(MainClass.provider_sql)
        modelos.Items.Clear()
        Rc.Open("select vehiculos_modelo from cavehiculos where vehiculos_estado='A'", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
        modelos.Items.Clear()
        Do While Rc.EOF = False
            modelos.Items.Add(Rc.Fields.Item("vehiculos_modelo").Value)
    ...
    'carga los datos del vehiculos para el form destino
    Public Sub cargaDatosVehiculo(ByRef codigo As Integer)
        ...
        Db.Close()
    End Sub
    'se encarga de grabar los datos del vehiculo
    ...
    'Graba los datos de clientes, se conecta a la bd 
    ...
    Public Sub buscaClientesExternos(ByRef criterio As String, ByRef lista As ListView)
        ...
        If criterio = "ALL" Then
            Rc.Open("select clientes_codigo, clientes_id, clientes_nombres, clientes_apellidop, clientes_apellidom, usuarios_nick from caClientes where clientes_estado='A' order by clientes_apellidop ASC", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
        Else
            Rc.Open("select clientes_codigo, clientes_id, clientes_nombres, clientes_apellidop, clientes_apellidom, usuarios_nick from caClientes where clientes_estado='A' and clientes_apellidop like '%" & UCase(criterio) & "%' order by clientes_apellidop ASC", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
        End If
    ...
    'llena la lista en el formulario de Seguimiento frmSeguimiento
    ...
    Public Sub grabaSeguimiento(ByRef fechaConsulta As Date, ByRef estadoSeg As String, ByRef usuario As String, ByRef observacion As String)
        Dim usuarioSeg As String

        usuarioSeg = ObtieneUsuarioSeguimiento(gVar.codCli)
        ...
        Rc.Fields.Item("seg_usuario").Value = usuarioSeg
    ...
    Public Function ObtieneUsuarioSeguimiento(ByVal codCliente As Integer) As String
        ...
        Db.Open(MainClass.provider_sql)
        Rc.Open("select usuarios_nick from caclientes where clientes_codigo=" & codCliente & "", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)

        If Rc.RecordCount > 0 Then
            Do While Rc.EOF = False
                res = Rc.Fields.Item("usuarios_nick").Value
                Rc.MoveNext()
            Loop
        End If
        Rc.Close()
    ...
    'llena la lista en el frmagenda, se conecta a la bd
    Public Sub llenaListaAgenda(ByRef lista As ListView, ByRef usuario As String)
        ...
        If gVar.utUsuarios = 1 Then
            Rc.Open("select caSeguimiento.seg_fechaConsulta, caSeguimiento.seg_estadoSeg, caSeguimiento.seg_usuario, caSeguimiento.seg_observacion, caClientes.clientes_apellidop, caClientes.clientes_apellidom, caClientes.clientes_nombres,caClientes.usuarios_nick from caSeguimiento, caClientes where caSeguimiento.seg_estado='A' and caClientes.clientes_estado='A' and caSeguimiento.clientes_codigo = caClientes.clientes_codigo ", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
        Else
    ...
    'Valida a los usuarios 1 = Clave aceptada 2 = Clave Incorrecta 3 = Usuario Incorrecto , se conecta a la bd
    ...
    'carga en en combo los modelos de carros
    ...
    'carga en un combo los tipos de usuarios
    ...
    'carga la lista de las cotizaciones, acceso a bd
    Public Sub cargaListaCotiza(ByRef codigo As Integer, ByRef lista As ListView)
        ...
        Db.Open(MainClass.provider_sql)
        lista.Items.Clear()
        Rc.Open("select * from cacotizacion where cot_estado='A' and clientes_codigo=" & codigo & " and num_cot=" & numeroCot & "", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
        If Rc.RecordCount > 0 Then
            t.SubItems.Clear()
            Do While Rc.EOF = False
                t.SubItems.Clear()
    ...
    'llena el combo de la lista de cotizaciones del cliente
    Sub cargaComboCotiza(ByRef codigo As Integer, ByRef combo As ComboBox)
        Dim Db As New ADODB.Connection
        Dim Rc As New ADODB.Recordset
        Dim t As New ListViewItem
        Dim x As Date
        Db.Open(MainClass.provider_sql)
        combo.Items.Clear()
        combo.Items.Add(" < Nueva Cotización >")
        Rc.Open("select num_cot,vendido from cacotizacion where clientes_codigo=" & codigo & " and cot_estado='A' group by num_cot,vendido", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)

        If Rc.RecordCount > 0 Then

            Do While Rc.EOF = False

                If Rc.Fields.Item("vendido").Value = 1 Then
                    combo.Items.Add(Rc.Fields.Item("num_cot").Value & "    ( *** Vendido  *** )")
                Else
                    combo.Items.Add(Rc.Fields.Item("num_cot").Value)
                End If
    ...
    'elimiuna una cotizacion(Pone la E en  la tabla cotizaciones) 
    ...
    'guarda una cotizacion nueva en la base de datos , tabla cacotizaciones
    Public Sub grabaCotizacion(ByRef primero As Boolean, ByRef codigoCli As Integer, ByRef codigoVeh As Integer, ByRef modelo As String, ByRef precio As Double, ByRef institucion As String, ByRef entrada As Double, ByRef plazo As Integer, ByRef cuota As Double, ByRef observacion As String, ByRef beneficios As String)

        If primero Then
            Dim Db1 As New ADODB.Connection
            Dim Rc1 As New ADODB.Recordset
            Db1.Open(MainClass.provider_sql)
            Rc1.Open("select max(num_cot) as maximo from cacotizacion where  cot_estado='A' ", Db1, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
            If Rc1.RecordCount > 0 Then
                Do While Rc1.EOF = False
                    numeroCot = Rc1.Fields.Item("maximo").Value
                    numeroCot = numeroCot + 1
                    Rc1.MoveNext()
                Loop
                Rc1.Close()
                Db1.Close()
                gVar.NumCot = numeroCot
        ...
        Rc.AddNew()
        Rc.Fields.Item("num_cot").Value = numeroCot

        Rc.Fields.Item("clientes_codigo").Value = codigoCli

        Rc.Fields.Item("vehiculos_codigo").Value = codigoVeh

        Rc.Fields.Item("codigo_usuario").Value = gVar.tempNickUser
        Rc.Fields.Item("Vendido").Value = 0     'por default le dice que no venda nada
    ...
    Function valida_que_tenga_Cotizacion(ByVal codigo As Double) As Boolean
        ...
        Rc.Open("select num_cot from caCotizacion where clientes_codigo=" & codigo & "", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
        If Rc.RecordCount > 0 Then
            Do While Rc.EOF = False
                res = 1
                Rc.MoveNext()
    ...
    Public Sub vender(ByRef clienteCodigo As Integer, ByRef cotizacionNum As Integer)
        ...
        cn.Open(MainClass.provider_sql)

        If gVar.Fecha_Asc = 1 Then
            rs = cn.Execute("update cacotizacion set Vendido=1,cot_fecha_venta='" & System.DateTime.Today.Day.ToString() & "/" & System.DateTime.Today.Month.ToString() & "/" & System.DateTime.Today.Year.ToString() & "'  where clientes_codigo=" & clienteCodigo & " and num_cot=" & cotizacionNum & " and cot_estado='A'")
        Else
            rs = cn.Execute("update cacotizacion set Vendido=1,cot_fecha_venta='" & System.DateTime.Today.Year.ToString() & "/" & System.DateTime.Today.Month.ToString() & "/" & System.DateTime.Today.Day.ToString() & "'  where clientes_codigo=" & clienteCodigo & " and num_cot=" & cotizacionNum & " and cot_estado='A'")
        ...
        MsgBox("Cotización Vendida...")

    End Sub


    Public Function vendido(ByRef clienteCodigo As Integer, ByRef cotizacionNum As Integer) As Boolean
        ...
        rs = cn.Execute("select  vendido  from cacotizacion where clientes_codigo=" & clienteCodigo & " and num_cot=" & cotizacionNum & " and cot_estado='A' group by vendido")

        resultado = rs.Fields(0).Value.ToString
        rs.Close()
        cn.Close()
    ...
    'carga de la base de datos las licencias, se conecta a la bd
    'MODIFIED  try
    ...
    Function obtiene_ultimo_codigo_de_caUsuarioTipo() As Integer
        ...
        Db.Open(MainClass.provider_sql)
        Rc.Open("select max(usuarioTipo_Codigo) as maximo from causuariostipo", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)

        If Rc.RecordCount > 0 Then
            Do While Rc.EOF = False
                res = Rc.Fields.Item("maximo").Value
                Rc.MoveNext()
            Loop
        End If
        Rc.Close()
    ...
    Public Sub creaTipos(ByRef nombre As String)
        ...
        Rc1.Fields.Item("usuarioTipo_Codigo").Value = obtiene_ultimo_codigo_de_caUsuarioTipo()
    ...
    'carga los datos de referencia de un cliente de la tabla carefernecias
    ...
    Public Sub SolicitudImprimir()
        Dim cn As ADODB.Connection
        cn = New ADODB.Connection()
        cn.Open(provider_sql)
        'cn.Execute("Insert into caSolicitudes(fecha_impreso,Codigo_Usuario,clientes_codigo,bancos_descripcion,solicitudes_Aprobado) values ('" & System.DateTime.Today.Year.ToString() & "/" & System.DateTime.Today.Month.ToString() & "/" & System.DateTime.Today.Day.ToString() & "','" & gVar.tempNickUser & "','" & gVar.tNombres & "','" & MainClass.busca_banco_por_ruta(gVar.bRuta) & "',0)")
        If gVar.Fecha_Asc = 1 Then
            cn.Execute("Insert into caSolicitudes(fecha_impreso,Codigo_Usuario,clientes_codigo,bancos_descripcion,solicitudes_Aprobado) values ('" & System.DateTime.Today.Day.ToString() & "/" & System.DateTime.Today.Month.ToString() & "/" & System.DateTime.Today.Year.ToString() & "','" & gVar.tempNickUser & "','" & gVar.tNombres & "','" & MainClass.busca_banco_por_ruta(gVar.bRuta) & "',null)")
        Else
            cn.Execute("Insert into caSolicitudes(fecha_impreso,Codigo_Usuario,clientes_codigo,bancos_descripcion,solicitudes_Aprobado) values ('" & System.DateTime.Today.Year.ToString() & "/" & System.DateTime.Today.Month.ToString() & "/" & System.DateTime.Today.Day.ToString() & "','" & gVar.tempNickUser & "','" & gVar.tNombres & "','" & MainClass.busca_banco_por_ruta(gVar.bRuta) & "',null)")
    ...
    Public Function busca_banco_por_ruta(ByRef bco As String) As String
        ...
        Rc.Open("select * from cabancos where bancos_ruta like '" & bco & "'", Db, ADODB.CursorTypeEnum.adOpenKeyset, ADODB.LockTypeEnum.adLockReadOnly)
        If Rc.RecordCount > 0 Then
            Do While Rc.EOF = False
                res = Rc.Fields.Item("bancos_descripcion").Value
                Rc.MoveNext()
            Loop
        End If
        Rc.Close()
